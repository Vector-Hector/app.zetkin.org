...
import { Components, createTheme, CssVarsTheme } from '@mui/material/styles';
...
const createThemeWithMode = (mode: ResolvedThemeMode) => {
  const palette = mode === 'dark' ? oldDarkThemePalette : oldThemePalette;
  return createTheme({
    components: {
      MuiButton: {
        styleOverrides: {
        ...
        styleOverrides: {
          root: {
            ...
            '& .MuiDataGrid-columnHeaderTitle': {
              color: palette.onSurface.medium,
              fontWeight: 'bold',
            },
            '& [role="toolbar"]': {
              '& > *': {
...
export const oldThemeWithLocale = (lang: string, mode?: ResolvedThemeMode) => {
  const resolvedMode =
    mode ?? getResolvedModeSettingFromLocalStorage() ?? 'light';

  return createTheme(createThemeWithMode(resolvedMode), locales[lang]);
};

export default createThemeWithMode(
  getResolvedModeSettingFromLocalStorage() ?? 'light'
);
